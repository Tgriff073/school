...
bool Dictionary::ProcessTransactionFile()
{
		...
		if(fin.fail())
		{
  		...
  		while(fin>>cmd)
  		{
  			...
  			{
  				fin>>word;
  				this->InsertWordsIntoDict(word);
  			}//end InsertF check
  			else if(cmd=="PrintF")
  			{
  				fin>>word;
  				if(this->PrintAFileInDict(word)==(Dictionary::failure))
  			 		cout<<word<<" Failed to print, file couldn't open"<<endl;     
  			...
  			else if(cmd=="DeleteW")
  			{
  				...
  				if(!this->DeleteAWord(word))
  					cout<<"Failed to delete "<<word<<endl;
  			}//end DeleteW check
  			else if(cmd=="SearchW")
  			{
  				fin>>word;
  			...
  			else if(cmd=="SpellCheck")
  			{
  				...
  				if(this->SpellChecking(word)==Dictionary::failure)
  					cout<<word<<" Could not be opened for spell checking."<<endl;
